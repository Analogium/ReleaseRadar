...
- [x] Balises **meta par page** : `title` + `meta description` uniques via un composant [`Seo`](frontend/src/components/Seo.tsx) (mise à jour impérative des balises, sans dépendance externe). Pages publiques indexables (accueil, login, register, légales) ; espace applicatif en **`noindex`**
- [x] **Open Graph** + **Twitter Cards** (titre, description, url) — statiques dans `index.html`, complétés par page via `Seo`. *Image de partage encore un placeholder (`TODO(SEO)`), à remplacer par un vrai visuel 1200×630 hébergé*
- [x] `lang="fr"` sur `<html>`, URL **canonique** (par page), favicon déjà présent
- [x] **`robots.txt`** (routes privées en `Disallow`) + **`sitemap.xml`** des pages publiques ([`frontend/public/`](frontend/public/))
- [x] **JSON-LD** (`WebApplication`) dans `index.html` pour les rich results

#### 16.2 — Contenu & accessibilité (bon pour SEO *et* a11y)

- [x] HTML sémantique sur la landing (un seul `<h1>`, hiérarchie de titres cohérente)
- [x] `alt` descriptifs sur les pochettes (Cover Art), ex. « Pochette de … par … »
- [x] Une vraie **page d'accueil / landing publique** ([`Landing.tsx`](frontend/src/pages/Landing.tsx)) : `/` est désormais la vitrine (pitch + CTA), accessible à tous ; les CTA s'adaptent selon l'état de connexion (lien vers `/dashboard` pour les connectés). Le tableau de bord a migré sur `/dashboard`

#### 16.3 — Performance (Core Web Vitals)

- [x] **Code-splitting des routes** (`React.lazy` + `Suspense`, [`App.tsx`](frontend/src/App.tsx)) : la vitrine reste dans le bundle principal (LCP), chaque autre page charge son propre chunk. Vendor (React/Router/axios) isolé dans un chunk `vendor` cacheable. Résultat build : entrée app **364 kB → 23 kB** (+ `vendor` 288 kB en cache longue durée), pages app hors du chargement initial
- [x] **Lazy-loading des images** : `loading="lazy"` sur les pochettes (`CoverArt`) + `preconnect`/`dns-prefetch` vers la Cover Art Archive. Compression `zstd/gzip` assurée par Caddy
- [ ] Audit **Lighthouse** (perf, SEO, a11y, best practices) — *corrections proactives déjà appliquées (splitting, lazy, preconnect, meta) ; reste à lancer un audit formel sur la prod et itérer*
- [ ] *(si SEO critique, non retenu)* **prerendering** des pages publiques (SSG) — une migration SSR complète (Next/Remix) serait lourde et peu justifiée vu que le cœur applicatif est privé ; l'`index.html` porte déjà les meta/OG/JSON-LD de base pour la vitrine
